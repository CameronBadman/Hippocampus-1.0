...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the non-sealed Spider v0.5 partitions."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/spider_v0_5/splits"),
    )
...
def main() -> None:
    ...
    for spec in default_score_decode_specs():
        if spec.sealed:
            raise RuntimeError("this command may not materialise sealed data")
        cases = generate_score_decode_cases(spec)
        ...
        if not all_case_ids.isdisjoint(manifest.case_ids):
            raise RuntimeError("case identity crossed v0.5 partitions")
        if not all_base_ids.isdisjoint(manifest.base_case_ids):
            raise RuntimeError("base identity crossed v0.5 partitions")
    ...
    base_index = {
        ...
        "purpose": "score-versus-current-candidate-decoding factorial",
